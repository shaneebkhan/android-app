...
class PosOrderLineLot(models.Model):
    ...
    def create(self, values):
        if values.get('server_id'):
            lot = self.search([('id', '=', values.get('server_id'))])
...
class PosOrder(models.Model):
    ...
    def get_table_draft_orders(self, table_id):
        ...
        def _get_pack_lot_lines(order_lines):
            ...
            for order_line_id, pack_lot_ids in groupby(pack_lots, key=lambda x:x['pos_order_line_id']):
                next(order_line for order_line in order_lines if order_line['id'] == order_line_id)['pack_lot_ids'] = list(pack_lots)

        def _get_order_lines(orders):
            ...
            if order_lines != []:
                _get_pack_lot_lines(order_lines)
        ...
        def _get_payment_lines(orders):
            ...
            :type orders: pos.order.
            """
            payment_lines = self.env['account.bank.statement.line'].search_read(
                    domain = [('pos_statement_id', 'in', [to['id'] for to in orders])],
                    fields = [
                        'id',
                        'amount',
                        'pos_statement_id',
            ...
            for payment_line in payment_lines:
                ...
                payment_line['statement_id'] = payment_line['statement_id'][0]
                if 'line_type' in payment_line:
                    payment_line['type'] = payment_line['line_type']
                    del payment_line['line_type']

                del payment_line['id']
                extended_payment_lines.append([0, 0, payment_line])
            for order_id, payment_lines in groupby(extended_payment_lines, key=lambda x:x[2]['pos_statement_id']):
        ...
        _get_order_lines(table_orders)
        _get_payment_lines(table_orders)
