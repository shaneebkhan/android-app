...
QUnit.test('auto-update out-of-office info on im_status change', function (assert) {
    var done = assert.async();
    assert.expect(5);

    var imStatusDefs = [$.Deferred(), $.Deferred()];
    var channelInfoDefs = [$.Deferred(), $.Deferred()];
    ...
    assert.strictEqual($('.o_out_of_office').length, 0,
    ...
    $.when(imStatusDefs[0], channelInfoDefs[0]).then(function () {
        assert.strictEqual($('.o_out_of_office').length, 1,
            "should contain out of office section on chat window");
        assert.strictEqual($('.o_out_of_office > .o_out_of_office_text').length, 1,
        ...
        $.when(imStatusDefs[1], channelInfoDefs[1]).then(function () {
            assert.strictEqual($('.o_out_of_office').length, 0,
