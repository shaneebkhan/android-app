...
QUnit.test('chat window: basic rendering', async function (assert) {
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_chat_window_manager > .o_chat_window').length, 1, "should have open a chat window");
    assert.strictEqual(document.querySelector('.o_mail_wip_chat_window_manager > .o_chat_window').dataset.threadLocalId, 'mail.channel_20', "should have open a chat window of channel");
...
QUnit.test('open 2 different chat windows: enough screen width', async function (assert) {
    ...
    document.querySelector('.o_mail_wip_systray_messaging_menu > a').click();
    await testUtils.nextTick(); // re-render
    document.querySelector('.o_mail_wip_systray_messaging_menu > .dropdown-menu > .o_mail_wip_thread_preview_list > .o_preview[data-thread-local-id="mail.channel_10"]').click();
    await testUtils.nextTick(); // re-render

    assert.strictEqual(document.querySelectorAll('.o_mail_wip_chat_window_manager > .o_chat_window').length, 1, "should have open a chat window");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_chat_window_manager > .o_chat_window[data-thread-local-id="mail.channel_10"]').length, 1, "chat window of chat should be open");
    assert.ok(document.querySelector('.o_mail_wip_chat_window_manager > .o_chat_window[data-thread-local-id="mail.channel_10"]').classList.contains('o_focused'), "chat window of chat should have focus");

    document.querySelector('.o_mail_wip_systray_messaging_menu > a').click();
    await testUtils.nextTick(); // re-render
    document.querySelector('.o_mail_wip_systray_messaging_menu > .dropdown-menu > .o_mail_wip_thread_preview_list > .o_preview[data-thread-local-id="mail.channel_20"]').click();
    await testUtils.nextTick(); // re-render
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_chat_window_manager > .o_chat_window[data-thread-local-id="mail.channel_20"]').length, 1, "chat window of channel should be open");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_chat_window_manager > .o_chat_window[data-thread-local-id="mail.channel_10"]').length, 1, "chat window of chat should still be open");

    assert.ok(document.querySelector('.o_mail_wip_chat_window_manager > .o_chat_window[data-thread-local-id="mail.channel_20"]').classList.contains('o_focused'), "chat window of channel should have focus");
    assert.notOk(document.querySelector('.o_mail_wip_chat_window_manager > .o_chat_window[data-thread-local-id="mail.channel_10"]').classList.contains('o_focused'), "chat window of chat should no longer have focus");
});
...
QUnit.test('open 3 different chat windows: not enough screen width', async function (assert) {
    ...
    document.querySelector('.o_mail_wip_systray_messaging_menu > a').click();
    await testUtils.nextTick(); // re-render
    document.querySelector('.o_mail_wip_systray_messaging_menu > .dropdown-menu > .o_mail_wip_thread_preview_list > .o_preview[data-thread-local-id="mail.channel_1"]').click();
    await testUtils.nextTick(); // re-render
    ...
    document.querySelector('.o_mail_wip_systray_messaging_menu > a').click();
    await testUtils.nextTick(); // re-render
    document.querySelector('.o_mail_wip_systray_messaging_menu > .dropdown-menu > .o_mail_wip_thread_preview_list > .o_preview[data-thread-local-id="mail.channel_2"]').click();
    await testUtils.nextTick(); // re-render
    ...
    document.querySelector('.o_mail_wip_systray_messaging_menu > a').click();
    await testUtils.nextTick(); // re-render
    document.querySelector('.o_mail_wip_systray_messaging_menu > .dropdown-menu > .o_mail_wip_thread_preview_list > .o_preview[data-thread-local-id="mail.channel_3"]').click();
    await testUtils.nextTick(); // re-render
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_chat_window_manager > .o_chat_window[data-thread-local-id="mail.channel_1"]').length, 1, "chat window of channel 1 should be open");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_chat_window_manager > .o_chat_window[data-thread-local-id="mail.channel_3"]').length, 1, "chat window of channel 3 should be open");
    assert.ok(document.querySelector('.o_mail_wip_chat_window_manager > .o_chat_window[data-thread-local-id="mail.channel_3"]').classList.contains('o_focused'), "chat window of channel 3 should have focus");
});
