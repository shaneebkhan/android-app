...
QUnit.test('basic rendering: sidebar', async function (assert) {
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_sidebar > .o_group.o_mailbox > .o_item[data-thread-local-id="mail.box_inbox"]').length, 1, "should have inbox mailbox item");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_sidebar > .o_group.o_mailbox > .o_item[data-thread-local-id="mail.box_starred"]').length, 1, "should have starred mailbox item");
...
QUnit.test('sidebar: basic mailbox rendering', async function (assert) {
    ...
    const inbox = document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_mailbox > .o_item[data-thread-local-id="mail.box_inbox"]');
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_inbox"] > .o_counter').length, 0, "should have no counter when equal to 0 (default value)");
});
...
QUnit.test('sidebar: change item', async function (assert) {
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_inbox"] > .o_active_indicator').classList.contains('o_active'), true, "inbox should be active by default");
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_starred"] > .o_active_indicator').classList.contains('o_active'), false, "starred should be inactive by default");

    await testUtils.dom.click(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_starred"]'));

    assert.strictEqual(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_inbox"] > .o_active_indicator').classList.contains('o_active'), false, "inbox mailbox should become inactive");
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_starred"] > .o_active_indicator').classList.contains('o_active'), true, "starred mailbox should become active");
});
...
QUnit.test('sidebar: inbox with counter', async function (assert) {
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_inbox"] > .o_counter').length, 1, "should have a counter when different from 0");
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_inbox"] > .o_counter').textContent, "100", "should have counter value");
});
...
QUnit.test('sidebar: public/private channel rendering', async function (assert) {
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item[data-thread-local-id="mail.channel_100"]').length, 1, "should have channel1 (ID 100)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item[data-thread-local-id="mail.channel_101"]').length, 1, "should have channel2 (ID 101)");

    const channel1 = document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item[data-thread-local-id="mail.channel_100"]');
    const channel2 = document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item[data-thread-local-id="mail.channel_101"]');
...
QUnit.test('sidebar: basic chat rendering', async function (assert) {
    ...
    assert.strictEqual(chat.dataset.threadLocalId, "mail.channel_10", "should have chat with ID 20");
...
QUnit.test('sidebar: chat im_status rendering', async function (assert) {
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_sidebar > .o_group.o_chat > .o_list > .o_item[data-thread-local-id="mail.channel_11"]').length, 1, "should have Partner1 (ID 11)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_sidebar > .o_group.o_chat > .o_list > .o_item[data-thread-local-id="mail.channel_12"]').length, 1, "should have Partner2 (ID 12)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_sidebar > .o_group.o_chat > .o_list > .o_item[data-thread-local-id="mail.channel_13"]').length, 1, "should have Partner3 (ID 13)");

    const chat1 = document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_chat > .o_list > .o_item[data-thread-local-id="mail.channel_11"]');
    const chat2 = document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_chat > .o_list > .o_item[data-thread-local-id="mail.channel_12"]');
    const chat3 = document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_chat > .o_list > .o_item[data-thread-local-id="mail.channel_13"]');
...
QUnit.test('sidebar: rename chat', async function (assert) {
    ...
    assert.strictEqual(chat.querySelector(':scope > .o_name').textContent, "Marc", "chat should have custom name as name");
    assert.notOk(chat.querySelector(':scope > .o_name').classList.contains('o_editable'), "chat name should not be editable");

    await testUtils.dom.click(chat.querySelector(':scope > .o_commands > .o_command.o_rename'), { allowInvisible: true });

    assert.ok(chat.querySelector(':scope > .o_name').classList.contains('o_editable'), "chat should have editable name");
    assert.strictEqual(chat.querySelectorAll(':scope > .o_name > input').length, 1, "chat should have editable name input");
    assert.strictEqual(chat.querySelector(':scope > .o_name > input').value, "Marc", "editable name input should have custom chat name as value by default");
    assert.strictEqual(chat.querySelector(':scope > .o_name > input').placeholder, "Marc Demo", "editable name input should have partner name as placeholder");

    chat.querySelector(':scope > .o_name > input').value = "Demo";
    const kevt = new window.KeyboardEvent('keydown', { key: "Enter" });
    chat.querySelector(':scope > .o_name > input').dispatchEvent(kevt);
    await testUtils.nextTick(); // re-render

    assert.notOk(chat.querySelector(':scope > .o_name').classList.contains('o_editable'), "chat should no longer show editable name");
...
QUnit.test('load single message from channel initially', async function (assert) {
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"]').length, 1, "should have message with ID 100");
});
...
QUnit.test('basic rendering of message', async function (assert) {
    ...
    const message = document.querySelector('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"]');
...
QUnit.test('basic rendering of squashed message', async function (assert) {
    ...
    const message1 = document.querySelector('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"]');
    const message2 = document.querySelector('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_101"]');
...
QUnit.skip('restore thread scroll position', async function (assert) {
    ...
    // select channel2
    await testUtils.dom.click(document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item[data-thread-local-id="mail.channel_2"]'));
    await testUtils.nextTick(); // re-render

    // select channel1
    await testUtils.dom.click(document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item[data-thread-local-id="mail.channel_1"]'));
    await testUtils.nextTick(); // re-render
...
QUnit.test('message origin redirect to channel', async function (assert) {
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message').length, 2, "should have 2 messages");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"]').length, 1, "should have message1 (ID 100)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_101"]').length, 1, "should have message2 (ID 101)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"] > .o_core > .o_header > .o_origin_thread').length, 0, "message1 should not have origin part in channel1 (same origin as channel)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_101"] > .o_core > .o_header > .o_origin_thread').length, 1, "message2 should have origin part (origin is channel2 !== channel1)");
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_101"] > .o_core > .o_header > .o_origin_thread').textContent.trim(), "(from #channel2)", "message2 should display name of origin channel");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_101"] > .o_core > .o_header > .o_origin_thread > a').length, 1, "message2 should have link to redirect to origin");

    // click on origin link of message2 (= channel2)
    document.querySelector('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_101"] > .o_core > .o_header > .o_origin_thread > a').click();
    await testUtils.nextTick(); // re-render

    assert.strictEqual(document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item[data-thread-local-id="mail.channel_2"] > .o_active_indicator').classList.contains('o_active'), true, "channel2 should be active channel on redirect from discuss app");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message').length, 2, "should have 2 messages");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"]').length, 1, "should have message1 (ID 100)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_101"]').length, 1, "should have message2 (ID 101)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"] > .o_core > .o_header > .o_origin_thread').length, 1, "message1 should have origin thread part (= channel1 !== channel2)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_101"] > .o_core > .o_header > .o_origin_thread').length, 0, "message2 should not have origin thread part in channel2 (same as current channel)");
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"] > .o_core > .o_header > .o_origin_thread').textContent.trim(), "(from #channel1)", "message1 should display name of origin channel");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"] > .o_core > .o_header > .o_origin_thread > a').length, 1, "message1 should have link to redirect to origin channel");
});
...
QUnit.test('redirect to author (open chat)', async function (assert) {
    ...
    assert.ok(document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item[data-thread-local-id="mail.channel_1"] > .o_active_indicator').classList.contains('o_active'), "channel 'General' should be active");
    assert.notOk(document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_chat > .o_list > .o_item[data-thread-local-id="mail.channel_10"] > .o_active_indicator').classList.contains('o_active'), "DM 'Demo' should not be active");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message').length, 2, "should have 2 messages");

    const msg1 = document.querySelector('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_100"]');
    const msg2 = document.querySelector('.o_mail_wip_discuss > .o_content > .o_thread > .o_message_list > .o_message[data-message-local-id="mail.message_101"]');
    ...
    assert.notOk(document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item[data-thread-local-id="mail.channel_1"] > .o_active_indicator').classList.contains('o_active'), "channel 'General' should become inactive after author redirection");
    assert.ok(document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_chat > .o_list > .o_item[data-thread-local-id="mail.channel_10"] > .o_active_indicator').classList.contains('o_active'), "chat 'Demo' should become active after author redirection");
});
...
QUnit.test('sidebar quick search', async function (assert) {
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss > .o_sidebar > .o_group.o_channel > .o_list > .o_item').dataset.threadLocalId, 'mail.channel_12', "should have filtered to a single channel item with ID 12");
...
QUnit.test('basic control panel rendering', async function (assert) {
    ...
    await testUtils.dom.click(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.channel_20"]'));
...
QUnit.test('inbox: mark all messages as read', async function (assert) {
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_inbox"] > .o_counter').textContent, "2", "inbox should have counter of 2");
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.channel_20"] > .o_counter').textContent, "2", "channel should have counter of 2");
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_inbox"] > .o_counter').length, 0, "inbox should display no counter (= 0)");
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.channel_20"] > .o_counter').length, 0, "channel should display no counter (= 0)");
...
QUnit.test('starred: unstar all', async function (assert) {
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_starred"] > .o_counter').textContent, "2", "starred should have counter of 2");
    ...
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_starred"] > .o_counter').length, 0, "starred should display no counter (= 0)");
...
QUnit.test('toggle_star message', async function (assert) {
    ...
    assert.strictEqual(document.querySelector('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_starred"] > .o_counter').textContent, "1", "starred should display a counter of 1");
    await pause();
    ...
    assert.verifySteps(['rpc:toggle_message_starred']);
    assert.strictEqual(document.querySelectorAll('.o_mail_wip_discuss .o_item[data-thread-local-id="mail.box_starred"] > .o_counter').length, 0, "starred should no longer display a counter (= 0)");
