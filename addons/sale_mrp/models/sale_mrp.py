            ...
            for s_line in self.sale_line_ids:
                ...
                if bom.type == 'phantom':
                    ...
                    for product_id in components:
                        ...
                        stock_moves = self.env['stock.move'].concat(*prod_moves)
                        average_price_unit += factor * self.env['product.product'].browse(product_id)._compute_average_price(prod_qty_done, prod_quantity, stock_moves)
