...
class AccountLuFaia1(models.TransientModel):
    ...
    def _prepare_account_structure(self, accounts):
        ...
        def _compute_opening_debit_credit():
            ...
            for aid, val in self._cr.fetchall():
                ...
                if val < 0:
                    debit_credit[aid]['opening_credit'] = '%.2f' % abs(val)
                    debit_credit[aid]['opening_debit'] = 0.0
                else:
                    debit_credit[aid]['opening_debit'] = '%.2f' % val
                    debit_credit[aid]['opening_credit'] = 0.0
        ...
        for account in accounts:
            ...
            account_list.append({
                ...
                'account_balance_date_from': balance_data,
                'account_balance_date_to': balance_data
            })
    ...
    def _prepare_tax_structure(self, taxes):
        tax_list = []

        for tax in taxes:
            tax_data = {
                'tax_type': 'TVA',#tax.type_tax_use,
                'tax_code': tax.id,
                'description': tax.name,
                'country': tax.company_id.country_id.code,
                'state_code': tax.company_id.state_id.code
            }
            # TODO: What happens when it is group of taxes or tax included in price
            if tax.amount_type == 'percent':
                tax_data['percent_amount'] = '%.2f' % tax.amount
            if tax.amount_type == 'fix':
                tax_data['fix_amount'] = '%.2f' % tax.amount
                tax_data['currency_code'] = tax.company_id.currency_id.name
                # TODO: currency_amount is amount expressed in company currency
                tax_data['currency_amount'] = '%.2f' % tax.company_id.currency_id.rate,
    ...
    def _prepare_tax_information(self, line):
        datas = []
    ...
    def _prepare_move_lines(self, move_lines):
        ...
        for line in move_lines:
            ...
            amount_vals = {
                'amount': '%.2f' % (line.debit if line.debit else line.credit),
            ...
            lines[line.move_id.id].append({
                ...
                'debit_amount': amount_vals if line.credit else {},
                'credit_amount': amount_vals if line.debit else {},
                'tax_information': self._prepare_tax_information(line)
            })
        return lines
    ...
    def _prepare_transaction_entries(self, move_lines):
        ...
        for move in move_lines.mapped('move_id'):
            move_data[move.journal_id.id] = []
            move_data[move.journal_id.id].append({
    ...
    def _prepare_general_ledger_structure(self, move_lines):
        ...
        for journal in move_lines.mapped('journal_id'):
            general_ledger_data['journals'].append({
                ...
                'type': journal.type,
                'transactions': move_data.get(journal.id) or []
           })
    ...
    def _prepare_invoice_line(self, invoice_line_ids):
        ...
        for line in invoice_line_ids:
            ...
            d = {
                ...
                'description': line.name,
                'amount': '%.2f' % line.price_subtotal_signed,
                'currency_code': line.currency_id.name if line.currency_id != line.company_id.currency_id else '',
                'currency_amount': '%.2f' % (line.currency_id.rate if line.currency_id != line.company_id.currency_id else 0),
                'exchange_rate': '%.8f' % rate[line.currency_id.id] if line.currency_id != line.company_id.currency_id else 0,
    ...
    def _prepare_invoice_tax_detail(self, invoice_id):
        datas = []
        for tax_line in invoice_id.tax_line_ids.filtered(lambda x:x.tax_id.type_tax_use != 'none'):
            ...
            datas.append({
                ...
                'tax_type': 'TVA', #tax_line.tax_id.description[:9],
    ...
    def _prepare_invoice_structure(self, invoices):
        ...
        for invoice in invoices:
            ...
            print("\n\nCCCCC", d.get('customer_info'), d['partner_id'])
