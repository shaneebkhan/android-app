            ...
            defs.push(this._rpc({
                ...
                params: {
                    ...
                    cover_properties: {
                        ...
                        'text_size_class': $el.attr('data-textSizeClass'),
                        'text_align_class': $el.attr('data-textAlignClass'),
                    },
                },
            }));
        }
...
options.registry.blog_cover = options.Class.extend({
    ...
    _setActive: function () {
        ...
        _.each(this.$el, function (el) {
            ...
            if ($el.is(':not([data-change])')) {
                $el.removeClass('d-none');

                _.each(['size','filters','text_size', 'text_align'], function (opt_name) {
                    var $opts = $el.find('[data-coverOpt="'+ opt_name +'"]');
                    var notAllowed = (self.$target.data('use_' + opt_name) != 'True');
            ...
            $el.find('.o_js_hide_regular').toggleClass('d-none', self.$target.is('.o_wblog_post_page_cover_regular'));
            $el.find('.o_js_show_regular').toggleClass('d-none', !self.$target.is('.o_wblog_post_page_cover_regular'));
        });
        ...
        this.$target.attr('data-coverClass', this.$el.find('.active[data-coverOpt="size"]').data('selectClass') || '');
        this.$target.attr('data-textSizeClass', this.$el.find('.active[data-coverOpt="text_size"]').data('selectClass') || '');
        this.$target.attr('data-textAlignClass', this.$el.find('.active[data-coverOpt="text_align"]').data('selectClass') || '');
