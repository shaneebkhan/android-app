...
class BlogPost(models.Model):
    ...
    teaser_short = fields.Text('Teaser Short', compute='_compute_teaser_short', inverse='_set_teaser_short')
    ...
    @api.multi
    @api.depends('content', 'teaser_manual')
    def _compute_teaser_short(self):
        for blog_post in self:
            if blog_post.teaser_manual:
                blog_post.teaser_short = blog_post.teaser_manual
            else:
                content = html2plaintext(blog_post.content).replace('\n', ' ')
                blog_post.teaser_short = content[:75] + '...'

    @api.multi
    def _set_teaser_short(self):
        for blog_post in self:
            blog_post.teaser_manual = blog_post.teaser
