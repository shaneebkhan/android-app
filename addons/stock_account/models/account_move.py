...
class AccountMove(models.Model):
    ...
    def _reverse_move_vals(self, default_values, cancel=False):
        ...
        if not cancel:
            move_vals['line_ids'] = [vals for vals in move_vals['line_ids'] if not vals[2]['display_type'] == 'anglo']
        return move_vals
    ...
    def button_cancel(self):
        ...
        self.mapped('line_ids').filtered(lambda line: line.display_type == 'anglo').unlink()
        return res
...
class AccountMoveLine(models.Model):
    ...
    display_type = fields.Selection(selection_add=[('anglo', 'Anglo Saxon')])
