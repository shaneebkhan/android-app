...
return AbstractRenderer.extend({
    ...
    /**
     * Returns an object used to style chart elements indepently from the datasets.
     *
     * @private
     * @returns {Object}
     */
     ...
     * that item (actually the event type is mousemove),
     * a tooltip with the full item text is displayed.
     *
     * @private
     * @param {MouseEvent} e
    ...
    _onlegendTooltipHover: function (e, legendItem) {
        // The string legendItem.text is an initial section of legendItem.fullText.
        // If the two coincide, no need to generate a tooltip.
        // If a tooltip for the legend already exists, it is good and don't need
        // to be recreated.
        ...
        var $container = this.$el.find('.o_graph_canvas_container');
        ...
        this.$legendTooltip = $('<div>', {
            ...
            css: {
                maxWidth: Math.max(chartAreaRight - chartAreaLeft - 50, 0) + 'px',
                top: (e.clientY - containerTop) + 'px',
            }
        });
        $container.append(this.$legendTooltip);
        this.$legendTooltip[0].style.left = chartAreaRight - this.$legendTooltip.innerWidth() + 'px';
    },
    /**
