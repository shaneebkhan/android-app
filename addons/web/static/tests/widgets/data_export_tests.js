...
QUnit.module('widgets', {
    ...
    QUnit.test('exporting all data in list view', async function (assert) {
        ...
        await testUtils.dom.click($('.modal .o_tree_column:contains(Description) .o_add_field'));
    ...
    QUnit.test('saving fields list when exporting data', async function (assert) {
        ...
        assert.strictEqual($('.modal .o_fields_list .o_export_field').length, 0,
    ...
    QUnit.test('Export dialog UI test', async function (assert) {
        ...
        $('.modal .o_export_search_input').val('Activities').trigger($.Event('keyup', {
            keyCode: 65,
        }));
        ...
        assert.strictEqual($('.modal .o_fields_list li').length, 1, "There should be one field in export field list.");
        // Remove field
        await testUtils.dom.click($('.modal .o_fields_list li .o_remove_field'));
        assert.strictEqual($('.modal .o_fields_list li').length, 0, "There shouldn't be any field in list");
        list.destroy();
    });
});
