...
QUnit.module('Views', {
    ...
    QUnit.test('editable list view: discard changes when error from the server', async function (assert) {
        assert.expect(2);
        ...
        assert.containsNone(list, '.o_data_row:eq(0) .o_data_cell:eq(0) input', "First cell should have toggled back to readonly");
        assert.strictEqual(list.$('.o_data_row:eq(0) .o_data_cell:eq(0)').html(), "yop", "First cell should get its initial value back");
    ...
    QUnit.test('display a warning if no record can be edited', async function (assert) {
        ...
        this.data.foo.fields.foo.required = true;
        var list = await createView({
            View: ListView,
            model: 'foo',
            data: this.data,
            ...
            services: {
                notification: NotificationService.extend({
                    notify: function (params) {
                        assert.strictEqual(params.type, "danger");
    ...
    QUnit.test('editable list view: multi edition on readonly list', async function (assert) {
        assert.expect(13);
        ...
        await testUtils.nextTick();
        assert.containsOnce(document.body, '.modal', "there should be an opened modal");
        assert.ok($('.modal').text().includes('2 selected'), "the number of records should be correctly displayed");
