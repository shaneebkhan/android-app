...
var StatementAction = AbstractAction.extend({
    ...
    willStart: function () {
        ...
        return def.then(function () {
                ...
                return promise.then(function (result) {
                        var title = result && result[0] ? result[0]['display_name'] : ''
    ...
    reload: function() {
        ...
        this.widgets = [];
        var promise = Promise.resolve()
        if (!self.model.context.no_reload_lines) {
            promise = this.model.reload()
        }
    ...
    _onAction: function (event) {
        ...
        this.model[_.str.camelize(event.name)](handle, event.data.data).then(function () {
            ...
            if (mode === 'inactive' && current_line.mode !== 'inactive') {
