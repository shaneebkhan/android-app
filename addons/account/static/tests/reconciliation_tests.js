...
QUnit.module('account', {
    ...
    QUnit.test('Reconciliation basic rendering', async function (assert) {
        ...
        await testUtils.dom.click(widget.$('.o_notebook .nav-link[href*="notebook_page_misc"]'));
        assert.strictEqual(widget.$el.data('mode'), 'create', "should switch to 'create' mode");

        await testUtils.dom.click(widget.$('.o_notebook .nav-link[href*="notebook_page_match"]'));
    ...
    QUnit.test('Tax on account receivable', async function(assert){
        ...
        assert.equal($($newLineGiftTds[0]).text().trim(), "101200",
        ...
        assert.equal($($newLineTaxeTds[0]).text().trim(), "101200",
