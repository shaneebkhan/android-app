...
class TestAccountMoveOutInvoice(AccountingSavepointCase):
    ...
    def test_out_invoice_line_onchange_product_1(self):
        for move_type in self.move_types:
            operational_field = 'credit' if move_type in ("out_invoice", "in_invoice") else 'debit'
            counterpart_field = 'debit' if move_type in ("out_invoice", "in_invoice") else 'credit'
            is_out = move_type in ('out_invoice', 'out_refund')
            ...
            self.assertAmlsValues(move.invoice_line_ids, [self.invariants[move_type]['line_balance']])
            self.assertAmlsValues(move.line_ids, [
                self.invariants[move_type]['line_balance'],
                self.invariants[move_type]['line_tax'],
                self.invariants[move_type]['line_product'],
            ...
            self.assertAmlsValues(move.invoice_line_ids, [
                {
                    **self.invariants[move_type]['line_product'],
                    ...
                    'account_id': self.parent_acc_revenue_2.id if is_out else self.parent_acc_expense_2.id,
                    'product_uom_id': self.uom_dozen.id,
                    'price_unit': 2000.0 if is_out else 1500.0,
                    'price_subtotal': 2000.0 if is_out else 1500.0,
                    'price_total': 2300.0 if is_out else 1725.0,
                    'tax_ids': self.parent_tax_sale_2.ids,
                    operational_field: 2000.0 if is_out else 1500.0,
                },
            ])
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -2300.0 if is_out else -1725.0,
                    'price_subtotal': -2300.0 if is_out else -1725.0,
                    'price_total': -2300.0 if is_out else -1725.0,
                    counterpart_field: 2300.0 if is_out else -1725.0,
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'name': self.parent_tax_sale_2.name if is_out else self.parent_tax_purchase_2.name,
                    'account_id': self.parent_tax_sale_2.account_id.id if is_out else self.parent_tax_purchase_2.account_id.id,
                    'price_unit': 300.0 if is_out else  225.0,
                    'price_subtotal': 300.0 if is_out else 225.0,
                    'price_total': 300.0 if is_out else 225.0,
                    'tax_line_id': self.parent_tax_sale_2.id if is_out else self.parent_tax_purchase_2.id,
                    operational_field: 300.0 if is_out else 225.0,
                },
                {
                    **self.invariants[move_type]['line_product'],
                    'name': 'product_b',
                    ...
                    'price_subtotal': 2000.0 if is_out else 1500.0,
                    'price_total': 2300.0 if is_out else 1725.0,
                    'tax_ids': self.parent_tax_sale_2.ids if is_out else self.parent_tax_purchase_2.ids,
                    operational_field: 2000.0 if is_out else 1500.0,
                },
            ])
            self.assertRecordValues(move, [{
                **self.invariants[move_type]['move'],
                'amount_untaxed': 2000.0 if is_out else 1500.0,
                'amount_tax': 300.0 if is_out else 225.0,
                'amount_total': 2300.0 if is_out else 1725.0,
                'amount_residual': 2300.0 if is_out else 1725.0,
            }])
    ...
    def test_out_invoice_line_onchange_quantity_1(self):
        for move_type in self.move_types:
            ...
            self.assertAmlsValues(move.invoice_line_ids, [
                {
                    **self.invariants[move_type]['line_product'],
                    'quantity': 2.0,
                    'price_subtotal': 2000.0 if is_out else 800.0,
                    'price_total': 2300.0 if is_out else 1600.0,
                    operational_field: 2000.0 if is_out else 1840.0,
                },
            ])
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -2300.0 if is_out else -1840.0,
                    'price_subtotal': -2300.0 if is_out else -1840.0,
                    'price_total': -2300.0 if is_out else -1840.0,
                    counterpart_field: 2300.0 if is_out else 1840.0,
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'price_unit': 300.0,
            ...
            self.assertRecordValues(move, [{
                **self.invariants[move_type]['move'],
                'amount_untaxed': 2000.0 if is_out else 1600.0,
                'amount_tax': 300.0 if is_out else 240.0,
                'amount_total': 2300.0 if is_out else 1840.0,
                'amount_residual': 2300.0 if is_out else 1840.0,
            }])
    ...
    def test_out_invoice_line_onchange_price_unit_1(self):
        for move_type in self.move_types:
            ...
            index_product_line, _ = self._search_candidate_records(move.line_ids, {'name': 'product_a'})
            with move_form.line_ids.edit(index_product_line) as line_form:
                line_form[operational_field] = 3000
            move = move_form.save()
    ...
    def test_out_invoice_line_onchange_discount_1(self):
        for move_type in self.move_types:
            ...
            self.assertAmlsValues(move.invoice_line_ids, [
                {
                    **self.invariants[move_type]['line_product'],
                    'discount': 50.0,
                    'price_unit': 1000.0 if is_out else 800.0,
                    'price_subtotal': 500.0 if is_out else 400.0,
                    'price_total': 575.0 if is_out else 460.0,
                    operational_field: 500.0 if is_out else 400.0,
                },
            ])
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -575.0 if is_out else -460.0,
                    'price_subtotal': -575.0 if is_out else -460.0,
                    'price_total': -575.0 if is_out else -460.0,
                    counterpart_field: 575.0 if is_out else -460.0,
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'price_unit': 75.0 if is_out else 60.0,
                    'price_subtotal': 75.0 if is_out else 60.0,
                    'price_total': 75.0 if is_out else 60.0,
                    operational_field: 75.0 if is_out else 60.0,
                },
                {
                    **self.invariants[move_type]['line_product'],
                    'discount': 50.0,
            ...
            self.assertRecordValues(move, [{
                **self.invariants[move_type]['move'],
                'amount_untaxed': 500.0 if is_out else 400.0,
                'amount_tax': 75.0 if is_out else 60.0,
                'amount_total': 575.0 if is_out else 460.0,
                'amount_residual': 575.0 if is_out else 460.0,
            }])
            ...
            self.assertAmlsValues(move.invoice_line_ids, [
                ...
                },
                {
                    **self.invariants[move_type]['line_product'],
                    'discount': 100.0,
                    'price_unit': 1000.0 if is_out else 500.0,
                    'price_subtotal': 0.0,
                    'price_total': 0.0,
                    counterpart_field: 0.0,
                },
            ...
            self.assertAmlsValues(move.line_ids, [
                ...
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'price_unit': 75.0 if is_out else 60.0,
                    'price_subtotal': 75.0 is_invoice else 60.0,
                    'price_total': 75.0 is_invoice else 60.0,
                    operational_field: 75.0 is_invoice else 60.0,
                },
                {
                    **self.invariants[move_type]['line_product'],
                    'name': 'product_a',
            ...
            with move_form.line_ids.edit(index_50discount_line) as line_form:
                line_form[operational_field] = 1000
            ...
            with move_form.line_ids.edit(index_100discount_line) as line_form:
                line_form.[operational_field] = 2000
            move = move_form.save()
            ...
            self.assertAmlsValues(move.invoice_line_ids, [
                {
                    **self.invariants[move_type]['line_product'],
                    'product_uom_id': self.uom_dozen.id,
                    'price_unit': 12000.0 if is_out else 9600.0,
                    'price_subtotal': 12000.0 if is_out else 9600.0,
                    'price_total': 13800.0 if is_out else 11040.0,
                    operational_field: 12000.0 if is_out else 9600.0,
                },
            ])
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -13800.0 if is_out else -11040.0,
                    'price_subtotal': -13800.0 if is_out else -11040.0,
                    'price_total': -13800.0 if is_out else -11040.0,
                    counterpart_field: 13800.0 if is_out else -11040.0,
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'price_unit': 1800.0 if is_oice else 1440.0,
                    'price_subtotal': 1800.0 if is_invoice else 1440.0,
                    'price_total': 1800.0 if is_invoice else 1440.0,
                    operational_field: 1800.0 if is_invoice else 1440.0,
                },
                {
                    **self.invariants[move_type]['line_product'],
                    'product_uom_id': self.uom_dozen.id,
                    'price_unit': 12000.0 if is_invoice else 9600.0,
                    'price_subtotal': 12000.0 if is_invoice else 9600.0,
                    'price_total': 13800.0 if is_invoice else 11040.0,
                    operational_field: 12000.0 if is_invoice else 9600.0,
                },
            ])
            self.assertRecordValues(move, [{
                **self.invariants[move_type]['move'],
                'amount_untaxed': 12000.0 if is_invoice else 9600.0,
                'amount_tax': 1800.0 if is_invoice else 1440.0,
                'amount_total': 13800.0 if is_invoice else 11040.0,
                'amount_residual': 13800.0 if is_invoice else 11040.0,
            }])
    ...
    def test_out_invoice_line_onchange_taxes_1_stackable_amounts(self):
        for move_type in self.move_types:
            operational_field = 'credit' if move_type in ("out_invoice", "in_invoice") else 'debit'
            counterpart_field = 'debit' if move_type in ("out_invoice", "in_invoice") else 'credit'
            is_invoice = move_type in ('out_invoice', 'out_refound')
            ...
            self.assertAmlsValues(move.invoice_line_ids, [
                {
                    **self.invariants[move_type]['line_product'],
                    'price_total': 1300.0 if is_invoice else 1040.0,
                    'tax_ids': (self.parent_tax_sale_1 + self.parent_tax_sale_2).ids if is_invoice else (self.parent_tax_purchase_1 + self.parent_tax_purchase_2).ids,
                },
            ])
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -1300.0 if is_invoice else 1040.0,
                    'price_subtotal': -1300.0 if is_invoice else 1040.0,
                    'price_total': -1300.0 if is_invoice else 1040.0,
                    counterpart_field: 1300.0 if is_invoice else 1040.0,
                },
                {
                    **self.invariants[move_type]['line_tax'],
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'name': self.parent_tax_sale_2.name if is_invoice else self.parent_tax_purchase_2.name,
                    'tax_line_id': self.parent_tax_sale_2.id if is_invoice else self.parent_tax_purchase_2.id,
                },
                {
                    **self.invariants[move_type]['line_product'],
                    'price_total': 1300.0 if is_invoice else 1040.0,
            ...
            self.assertRecordValues(move, [{
                **self.invariants[move_type]['move'],
                'amount_tax': 300.0 if is_invoice else 240.0,
                'amount_total': 1300.0 if is_invoice else 1040.0,
                'amount_residual': 1300.0 if is_invoice else 1040.0,
            }])
            ...
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -3900.0 if is_invoice else -3640.0,
                    'price_subtotal': -3900.0 if is_invoice else -3640.0,
                    'price_total': -3900.0 if is_invoice else -3640.0,
                    counterpart_field: 3900.0 if is_invoice else -3640.0,
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'price_unit': 450.0 if is_invoice else 420.0,
                    'price_subtotal': 450.0 if is_invoice else 420.0,
                    'price_total': 450.0 if is_invoice else 420.0,
                    'tax_line_id': self.parent_tax_sale_1.id if is_invoice else self.parent_tax_purchase_2.id,
                    operational_field: 450.0 if is_invoice else 420.0,
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'name': self.parent_tax_sale_2.name self.parent_tax_purchase_2.name,
                    'price_unit': 450.0 if is_invoice else 420.0,
                    'price_subtotal': 450.0 if is_invoice else 420.0,
                    'price_total': 450.0 if is_invoice else 420.0,
                    'tax_line_id': self.parent_tax_sale_1.id if is_invoice else self.parent_tax_purchase_2.id,
            ...
            self.assertRecordValues(move, [{
                **self.invariants[move_type]['move'],
                'amount_untaxed': 3000.0 if is_invoice else 2800.0,
                'amount_tax': 900.0 if is_invoice else 840.0,
                'amount_total': 3900.0 if is_invoice else 3640.0,
                'amount_residual': 3900.0 if is_invoice else 3640.0,
            }])
            ...
            with move_form.line_ids.edit(index_tax_line) as line_form:
                line_form[operational_field] = 600
            move = move_form.save()
            ...
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -4050.0 if is_invoice else -3820.0,
                    'price_subtotal': -4050.0 if is_invoice else -3820.0,
                    'price_total': -4050.0 if is_invoice else -3820.0,
                    counterpart_field: 4050.0 if is_invoice else -3820.0,
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'price_unit': 600.0,
            ...
            self.assertRecordValues(move, [{
                **self.invariants[move_type]['move'],
                'amount_untaxed': 3000.0 if is_invoice else 2800.0,
                'amount_tax': 1050.0 if is_invoice else 1020.0,
                'amount_total': 4050.0 if is_invoice else 3820.0,
                'amount_residual': 4050.0 if is_invoice else 3820.0,
            }])
            ...
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -3600.0 if is_invoice else -3400.0,
                    'price_subtotal': -3600.0 if is_invoice else -3400.0,
                    'price_total': -3600.0 if is_invoice else -3400.0,
                    counterpart_field: 3600.0 if is_invoice else -3400.0,
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    'price_unit': 600.0,
            ...
            self.assertRecordValues(move, [{
                **self.invariants[move_type]['move'],
                'amount_untaxed': 3000.0 if is_invoice else 2800.0,
                'amount_tax': 600.0,
                'amount_total': 3600.0 if is_invoice else 3400.0,
                'amount_residual': 3600.0 if is_invoice else 3400.0,
            }])
            ...
            self.assertAmlsValues(move.line_ids, [
                ...
                {
                    **self.invariants[move_type]['line_product'],
                    'price_unit': 150.0 if is_invoice else 120.0,
                    'price_subtotal': 150.0 if is_invoice else 120.0,
                    'price_total': 150.0 if is_invoice else 120.0,
                    operational_field: 150.0 if is_invoice else 120.0,
                },
                {
                    **self.invariants[move_type]['line_product'],
                    'name': self.parent_tax_sale_2.name if is_invoice else self.parent_tax_purchase_2.name,
            ...
            self.assertRecordValues(move, [{
                **self.invariants[move_type]['move'],
                'amount_untaxed': 1000.0 if is_invoice else 800.0,
                'amount_tax': 300.0 if is_invoice else 240.0,
                'amount_total': 1300 if is_invoice else 1040.0,
                'amount_residual': 1300.0 if is_invoice else 1040.0,
            }])
    ...
    def test_out_invoice_line_onchange_taxes_2_price_include(self):
        for move_type in self.move_types:
            ...
            with move_form.invoice_line_ids.new() as line_form:
                ...
                line_form.tax_ids.add(self.parent_tax_sale_1_incl if is_invoice else self.parent_tax_purchase_1_incl)
            move = move_form.save()
            ...
            self.assertAmlsValues(move.invoice_line_ids, [
                {
                    **self.invariants[move_type]['line_product'],
                    'price_unit': 1150.0,
                    'price_total': 1150.0,
                    'tax_ids': self.parent_tax_sale_1_incl.ids if is_invoice else self.parent_tax_purchase_1_incl.ids,
                },
            ])
            self.assertAmlsValues(move.line_ids, [
                {
                ...
                },
                {
                    **self.invariants[move_type]['line_tax'],
                    ...
                    'tax_line_id': self.parent_tax_sale_1_incl.id if is_invoice else self.parent_tax_purchase_1_incl.id,
                },
                {
                    **self.invariants[move_type]['line_product'],
                    'price_unit': 1150.0,
    ...
    def test_out_invoice_line_onchange_taxes_3_exigibility_on_payment(self):
        for move_type in self.move_types:
            ...
            self.assertAmlsValues(move.invoice_line_ids, [
                {
                    **self.invariants[move_type]['line_product'],
                    'tax_ids': self.parent_tax_sale_1_not_exigible.ids if is_invoice else self.parent_tax_sale_1_not_exigible.ids,
                    'tax_exigible': False,
                },
            ])
            self.assertAmlsValues(move.line_ids, [
                ...
                {
                    **self.invariants[move_type]['line_tax'],
                    'name': self.parent_tax_sale_1_not_exigible.name if is_invoice else self.parent_tax_purchase_1_not_exigible.name,
                    'tax_line_id': self.parent_tax_sale_1_not_exigible.id if is_invoice else self.parent_tax_purchase_1_not_exigible.id,
                },
                {
                    **self.invariants[move_type]['line_product'],
                    'tax_ids': self.parent_tax_sale_1_not_exigible.ids if is_invoice else self.parent_tax_sale_1_not_exigible.ids,
    ...
    def test_out_invoice_onchange_payment_term_1(self): 
        for move_type in self.move_types:
            ...
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -345.0 if is_invoice else -276.0,
                    'price_subtotal': -345.0 if is_invoice else -276.0,
                    'price_total': -345.0 if is_invoice else -276.0,
                    counterpart_field: 345.0 if is_invoice else -276.0,
                },
                {
                    **self.invariants[move_type]['line_balance'],
                    'price_unit': -805.0 if is_invoice else -644.0,
                    'price_subtotal': -805.0 if is_invoice else -644.0,
                    'price_total': -805.0 if is_invoice else -644.0,
                    counterpart_field: 805.0 if is_invoice else -644.0,
            ...
            with move_form.line_ids.edit(index_pay_term_line) as line_form:
                ...
                line_form.account_id = self.parent_acc_receivable_2 if is_invoice else self.parent_acc_payable_2
            move = move_form.save()
    ...
    def test_out_invoice_onchange_currency_1(self):
        for move_type in self.move_types:
            ...
            self.assertAmlsValues(move.line_ids, [
                {
                    **self.self.invariants[move_type]['line_balance'],
                    'price_unit': -4600.0,
    ...
    def test_out_invoice_create_invoice_line_ids_1_single_currency(self):
        for move_type in self.move_types:
            ...
            move = self.env['account.move'].with_context(default_type=move_type.create({
                'type': 'out_invoice',
