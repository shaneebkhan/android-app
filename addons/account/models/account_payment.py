...
class account_payment(models.Model):
    ...
    def default_get(self, fields):
        ...
        amount = self._compute_payment_amount(invoices=invoices, currency=invoices[0].currency_id, company=invoices[0].company_id)
        rec.update({
    ...
    @api.multi
    def _compute_payment_amount(self, invoices=None, currency=None, company=None, date=None):
...
class payment_register(models.TransientModel):
    ...
    def _prepare_payment_vals(self, invoice):
        ...
        amount = self.env['account.payment']._compute_payment_amount(
            invoices=invoice, currency=invoice.currency_id, company=self.journal_id.company_id, date=self.payment_date)
        values = {
