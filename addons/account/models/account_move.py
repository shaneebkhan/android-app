...
class AccountMove(models.Model):
    ...
    def _recompute_tax_lines(self, lines_map):
        ...
        for grouping_key, values in taxes_map.items():
            ...
            if not values['alive']:
                ...
                tax_line_vals.update(self._get_tax_line_values_from_base_lines(values['base_lines']))
                tax_line = self.env['account.move.line'].new(tax_line_vals)
                lines_map['tax_lines'] += tax_line
    ...
    def _recompute_cash_rounding_lines(self, lines_map, totals_map):
        ...
        exist in the currency. For the CHF, the smallest coin is 0.05 CHF.
        ...
        if self.invoice_cash_rounding_id:
            ...
            if rounding_lines and \
                    (strategy == 'biggest_tax' and not rounding_lines.tax_line_id) \
                    or (strategy == 'add_invoice_line' and rounding_lines.tax_line_id):
    ...
    def _recompute_payment_terms_lines(self, lines_map, totals_map):
        ...
        :param totals_map: A dictionary having two keys:
            ...
            * total_amount_currency:    Sum(amount_currency) of each product / tax line.
        '''
        # Keep in memory a mapping of each existing payment term line using its maturity date in order to retrieve
        # and update the existing lines. In case of multiple lines sharing the same maturity date, only the last one
        # will be updated.
        terms_map = {}
        for term_line in lines_map['terms_lines']:
            terms_map[fields.Date.to_string(term_line.date_maturity)] = term_line
        ...
        # Computation are done both in company's currency / foreign currency in order to ensure a consistency even with
    ...
    def _recompute_dynamic_lines(self, recompute_all_taxes=False):
        ...
        for line in self.line_ids:
            ...
            elif is_invoice and line.is_rounding_line:
                lines_map['rounding_lines'] += line
            elif is_invoice and not line.exclude_from_invoice_tab:
                lines_map['base_lines'] += line
            elif is_invoice and line.account_id.user_type_id.type in ('receivable', 'payable'):
    ...
    # TODO: if invoice_line_ids: recompute_dynamic_lines
    @api.multi
    def write(self, vals):
        vals.pop('invoice_line_ids', None)
    ...
    def _get_cash_basis_matched_percentage(self):
        ...
        if float_is_zero(total_amount, precision_rounding=self.company_id.currency_id.rounding):
            ...
            return total_reconciled / total_amount
        ...
        for line_command in move_vals.get('line_ids', 0.0):
            ...
            if line_vals.get('tax_repartition_line_id') and move_vals['type'] in ('out_refund', 'in_refund'):
                ...
                line_vals.update({
                    ...
                    'account_id': self.env['account.move.line']._get_default_tax_account(new_tax_repartition_line).id,
                    'tag_ids': [(6, 0, new_tax_repartition_line.tag_ids.ids)],
                })
        return move_vals
...
class AccountMoveLine(models.Model):
    ...
    # TODO: move payment_id to the move
    ...
    def _get_computed_taxes(self):
        ...
        if self.move_id.type in ('out_invoice', 'out_refund', 'out_receipt'):
            ...
            elif not self.exclude_from_invoice_tab:
    ...
    def _get_inversed_accounting_vals(self, price_unit, quantity, discount, balance, move_type, currency, taxes):
        ...
        if taxes and any(tax.price_include for tax in taxes):
            ...
            # 10            |               | 10% incl          | 10                | 10
            # 5             |               | 5%                | 5                 | 5
            taxes_res = taxes.compute_all(balance, currency=currency, handle_price_include=False)
    ...
    def _onchange_currency(self):
        for line in self:
            if line.move_id.type in ('out_invoice', 'out_refund', 'in_invoice', 'in_refund', 'out_receipt', 'in_receipt'):
