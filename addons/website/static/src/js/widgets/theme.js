            ...
            _.each($items, function (item) {
                ...
                switch (item.tagName) {
                    case 'OPT':
                        ...
                        var checked = widgetName === 'auto' ?
                            self.style.getPropertyValue('--' + $item.data('variable')).trim() === $item.attr('data-value') :
                            xmlid === undefined || xmlid && !_.difference(self._getXMLIDs($item), data.enabled).length;
