...
class TestNestedTaskUpdate(TestSaleOrder):
    ...
    def test_creating_subtask_user_id_on_parent_goes_on_child(self):
    ...
    #----------------------------------------
    #
    #   When writing on a parent task, some values adapt on theit children
    #
    #----------------------------------------
    ...
    def test_linking_user_id_on_parent_write_on_child(self):
        parent = self.env['project.task'].create({'name': 'parent', 'user_id': self.user.id})
