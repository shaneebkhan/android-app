...
class SendSMS(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if not self.env.context.get('default_recipients') and active_model and hasattr(self.env[active_model], '_get_default_sms_recipients'):
            ...
            for partner in partners:
                number = self.env['sms.api']._sanitize_record_number(partner, self.env.context.get('field_name') or 'mobile', 'country_id')
                if number:
    ...
    def action_send_sms(self):
        ...
        if active_model and hasattr(self.env[active_model], '_message_sms'):
            model = self.env[active_model]
            records = self._get_records(model)
            records[0]._message_sms(self.message, numbers=numbers)
        else:
