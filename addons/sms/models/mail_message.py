...
class MailMessage(models.Model):
    ...
    def _format_mail_failures(self):
        ...
        for message in self:
            ...
            if not any(notification.is_sms for notification in notifications):
                continue
            info = dict(message._get_mail_failure_dict(),
                        failure_type='sms',
                        notifications=dict((notif.res_partner_id.id, (notif.email_status, notif.res_partner_id.name)) for notif in notifications if notif.is_sms),
    ...
    def message_format(self):
        ...
        failed_sms_notifications = self.env['mail.notification'].sudo().search([
            ('mail_message_id', 'in', [r['id'] for r in message_values]),
            ('is_sms', '=', True),
            ('email_status', '!=', 'sent')
        ])
        msgid_to_notif = dict((notif.mail_message_id.id, notif) for notif in failed_sms_notifications)
        for message in message_values:
            customer_sms_data = [(notif.id, notif.res_partner_id.display_name, notif.email_status) for notif in msgid_to_notif.get(message['id'], [])]
