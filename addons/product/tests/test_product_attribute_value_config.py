...
class TestProductAttributeValueConfig(TestProductAttributeValueSetup):
    ...
    def test_constraints(self):
        ...
        with self.assertRaises(UserError, msg="can't change type of attribute used on product"):
        ...
        with self.assertRaises(UserError, msg="can't change the attribute of an attribute value"):
            self.ram_32.attribute_id = self.hdd_attribute.id

        with self.assertRaises(UserError, msg="can't delete attribute value used on product"):
            self.ram_32.unlink()
        ...
        with self.assertRaises(ValidationError, msg="value must match attribute"):
            self.env['product.template.attribute.line'].create({
                'product_tmpl_id': self.computer_case.id,
                'attribute_id': self.ram_attribute.id,
