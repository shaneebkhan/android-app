...
publicWidget.registry.ProductWishlist = publicWidget.Widget.extend(VariantMixin, {
    ...
    /**
     * Get the displayed nav button.
     * @private
     */
    _getNavBarButton: function (selector) {
        return $('.affixed ' + selector).length ? $('.affixed ' + selector) : $(selector).first();
    },
     ...
     * @private
     */
    _removeWish: function (e, deferred_redirect, animateTo) {
        ...
        if (animateTo) {
            wSaleUtils.animateClone(animateTo, tr, 25, 40, function () {
                Promise.resolve(rpc).then(function () {
                    $(tr).hide();
                });
            });
        }
    ...
    _addOrMoveWish: function (e) {
        ...
        if ($('#b2b_wish').is(':checked')) {
            ...
            this._removeWish(e, adding_deffered, navButton);
