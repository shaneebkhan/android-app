...
class pos_cache(models.Model):
    ...
    def refresh_cache(self):
        for cache in self:
            Product = self.env['product.product'].sudo(cache.compute_user_id.id)
            products = Product.search(cache.get_product_domain())
            prod_ctx = products.with_context(pricelist=cache.config_id.pricelist_id.id, display_default_code=False,
