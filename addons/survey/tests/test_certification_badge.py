...
class TestCertificationBadge(common.SurveyCase, HttpCase):
    def setUp(self):
        ...
        self.certification_survey = self.env['survey.survey'].sudo(self.survey_manager).create({
        ...
        self.certification_survey_2 = self.env['survey.survey'].sudo(self.survey_manager).create({
        ...
        self.certification_badge = self.env['gamification.badge'].sudo(self.survey_manager).create({
        ...
        self.certification_badge_2 = self.env['gamification.badge'].sudo(self.survey_manager).create({
        ...
        self.certification_badge_3 = self.env['gamification.badge'].sudo(self.survey_manager).create({
    ...
    def test_certification_badge_access(self):
        self.certification_badge.sudo(self.survey_manager).write(
        ...
        self.certification_badge.sudo(self.survey_user).write({'description': "Youpie Yeay!"})
        with self.assertRaises(AccessError):
            self.certification_badge.sudo(self.user_emp).write({'description': "I'm a dude who think that has every right on the Iron Throne"})
        with self.assertRaises(AccessError):
            self.certification_badge.sudo(self.user_portal).write({'description': "Guy, you just can't do that !"})
        with self.assertRaises(AccessError):
            self.certification_badge.sudo(self.user_public).write({'description': "What did you expect ? Schwepps !"})
