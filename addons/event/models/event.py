...
class EventEvent(models.Model):
    ...
    def _compute_date_begin_tz(self):
        for event in self:
            if event.date_begin:
                event.date_begin_located = format_tz(event.with_context(use_babel=True).env, event.date_begin, tz=event.date_tz)
            else:
    ...
    def _compute_date_end_tz(self):
        for event in self:
            if event.date_end:
                event.date_end_located = format_tz(event.with_context(use_babel=True).env, event.date_end, tz=event.date_tz)
            else:
