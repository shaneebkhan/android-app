...
class IrModuleModule(models.Model):
    ...
    #     upstream_theme_deps = upstream_deps.filtered(lambda mod: any(mod.name.startswith(keyword) for keyword in ('theme_')))
    #     return upstream_theme_deps

    def copy_already_installed_theme(self):
        current_website = self.env['website'].get_current_website()

        graph = odoo.modules.graph.Graph()


        to_copy = current_website.theme_id.upstream_dependencies(exclude_states=('',)).filtered(lambda x: x.state == 'installed')
        print(">> to_copy", to_copy)
        to_skip = current_website.theme_id.upstream_dependencies(exclude_states=('installed',)).filtered(lambda x: not x.name.startswith('theme_'))
        print(">> to_skip", to_skip)

        with self.env.registry.cursor() as cr:
            #graph.add_modules(cr, to_copy.mapped('name'), True)
            graph.add_modules(cr, ['base'], [])
            load_module_graph(cr, graph, None, perform_checks=False, report=None, models_to_check=None, skip_modules=to_skip.mapped('name'))
    ...
    def button_choose_theme(self):
        ...
        self.copy_already_installed_theme()

        
        next_action = self.button_immediate_install() # Then install the new chosen one

        #self._handle_auto_installs()
        _logger.info('installed themes %s on %s', current_website.depending_theme_ids.mapped('name'), current_website.name)
    ...
    def button_uninstall(self):
        ...
        #     current_website.theme_id -= modules_belonging_to_theme
        ...
        #         website.theme_id -= uninstalled_modules
